...
class Polygons:
    def __init__(self, poly_type: int):
        ...
        if self.points.shape[0] >= 1:
            ...
            if hasattr(self, 'poly_type'):
                ...
                if self.poly_type == 0:
                    ...
                    fill_color = (255, 0, 0)
            cv2.polylines(image, [pts], isClosed=closed, color=fill_color, thickness=thickness)
        return image
...
class UI(QWidget):
                            ...
                            if p.contains_point(point):
                                if p.poly_type ==0:
                                    ...
                                    if timer_key not in timers:
                                        timers[timer_key] = (current_time, True)
                                    elif not timers[timer_key][1]:
                                        # Resume timer if it was paused
                                        timers[timer_key] = (current_time, True)
                                    
                                    # Calculate elapsed time - this will update every frame
                                    elapsed_time = current_time - timers[timer_key][0]
                                    
                                    # Format time as seconds or minutes:seconds
                                    if elapsed_time < 60:
                                        time_text = f"{elapsed_time:.1f}s"
                                    else:
                                        minutes = int(elapsed_time // 60)
                                        seconds = elapsed_time % 60
                                        time_text = f"{minutes}m:{seconds:.0f}s"
                                    
                                    # Render timer on top of the point
                                    cv2.putText(frame, time_text, 
                                               (int(point[0]), int(point[1]) - 10),  # Slightly above the point
                                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
                            else:
                                # If point is not in this polygon but timer was active, stop it
                                if timer_key in timers and timers[timer_key][1] and p.poly_type == 1:
                                    # Mark timer as inactive
                                    start_time, _ = timers[timer_key]
                                    timers[timer_key] = (start_time, False)
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        elif event.text() in ('0', '1', '2'):
            if polygons:
                polygons[len(polygons)-1].poly_type=int(event.text())
